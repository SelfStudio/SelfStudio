...
import { getLocalizedAppContent } from "@/lib/i18n";
    ...
    <div className="relative min-h-screen">
              ...
              return (
                ...
                >
                  ...
                  <span className="mt-1.5 inline-flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-[0.06em] text-terracotta">
                    {dict.home.learnMore}
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="rtl:rotate-180">
      ...
      <footer className="border-t border-hairline">
        <div className="mx-auto flex max-w-[1160px] flex-wrap items-center justify-between gap-5 px-10 py-10 font-mono text-xs text-ink-3">
          <Link href={`/${app.id}/privacy/`} className="text-ink-3 no-underline hover:text-terracotta">
            {dict.privacy.title}
          </Link>
          <span>© {new Date().getFullYear()} SELFSTUDIO · SELFSTUDIO.FUN</span>
        </div>
      </footer>
    </div>
  );
